...
export default function AudioPage() {
  ...
  const fetchTracks = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://carefree-endurance-production-7621.up.railway.app'}/audio`);
      if(res.ok) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      if (uploadMethod === 'file') {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://carefree-endurance-production-7621.up.railway.app'}/audio`, {
          method: 'POST',
          headers: { 
  ...
  const handleDelete = async (id: string) => {
    const token = localStorage.getItem('adminToken');
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || 'https://carefree-endurance-production-7621.up.railway.app'}`}/audio/${id}`, {
      method: 'DELETE',
